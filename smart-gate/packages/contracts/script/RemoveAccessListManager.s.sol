...
contract RemoveAccessListManager is Script {
  using SmartGateUtils for bytes14;
  using SmartGateLib for SmartGateLib.World;

  SmartGateLib.World smartGate;

  function run(address worldAddress, address newAccessListManager) external {
    ...
    AccessListManager.deleteRecord(newAccessListManager);
    console.log("Access List Manager removed");
    console.logAddress(newAccessListManager);
